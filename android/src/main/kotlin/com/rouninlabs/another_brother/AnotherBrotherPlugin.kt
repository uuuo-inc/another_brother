...
class AnotherBrotherPlugin : FlutterPlugin, MethodCallHandler {
    ...
    override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
        if (call.method == "getPlatformVersion") {
            ...
            V4PrintImageMethodCall(
                flutterAssets = mFlutterAssets, context = mContext, call = call, result = result
            ).execute()
        } else if (call.method == StartCommunicationMethodCall.METHOD_NAME) {
            StartCommunicationMethodCall(
                flutterAssets = mFlutterAssets, context = mContext, call = call, result = result
            ).execute()
        } else if (call.method == EndCommunicationMethodCall.METHOD_NAME) {
            EndCommunicationMethodCall(
                flutterAssets = mFlutterAssets, context = mContext, call = call, result = result
            ).execute()
         ...
         */
            ...
            if (Build.VERSION.SDK_INT > Build.VERSION_CODES.M) {
                GetPdfFilePagesMethodCall(
                    flutterAssets = mFlutterAssets, context = mContext, call = call, result = result
                ).execute()
            } else {
                GetPdfPagesMethodCall(
                    flutterAssets = mFlutterAssets, context = mContext, call = call, result = result
                ).execute()
            }
